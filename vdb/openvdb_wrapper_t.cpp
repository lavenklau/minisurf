...
//const imath_half_uif_t* imath_half_to_float_table;
...
void openvdb_wrapper_t<float>::meshFromFile(
	const std::string& filename, std::vector<std::array<float, 3>>& points,
	std::vector<std::array<int, 3 >>& trias, std::vector<std::array<int, 4 >>& quads, double isovalue, bool relaxDisorientTri)
{
	using Scalar = float;
	openvdb::initialize();
	typedef typename std::conditional<std::is_same<Scalar, float>::value, openvdb::FloatGrid, openvdb::DoubleGrid>::type Grid;
	...
	for (int i = 0; i < points_vdb.size(); i++) {
		points.emplace_back(std::array<float, 3 >{(float)points_vdb[i][0], (float)points_vdb[i][1], (float)points_vdb[i][2]});
	}
	for (int i = 0; i < tris_vdb.size(); i++) {
		trias.emplace_back(std::array<int, 3 >{(int)tris_vdb[i][0], (int)tris_vdb[i][1], (int)tris_vdb[i][2]});
	}
	for (int i = 0; i < quads_vdb.size(); i++) {
		quads.emplace_back(std::array<int, 4>{(int)quads_vdb[i][0], (int)quads_vdb[i][1], (int)quads_vdb[i][2], (int)quads_vdb[i][3]});
	}
}
	...
	std::vector<std::array<int, 4>>& quads, double isovalue)
{
	...
	for (int i = 0; i < points_vdb.size(); i++) {
		points.emplace_back(std::array<float, 3 >{points_vdb[i][0], points_vdb[i][1], points_vdb[i][2]});
	}
	for (int i = 0; i < quads_vdb.size(); i++) {
		quads.emplace_back(std::array<int, 4 >{(int)quads_vdb[i][0], (int)quads_vdb[i][1], (int)quads_vdb[i][2], (int)quads_vdb[i][3]});
	}
