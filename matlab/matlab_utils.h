...
template<typename R>
struct IsComplexScalar {
	static constexpr mxComplexity complexFlag = mxREAL;
};

template<typename R>
struct IsComplexScalar<std::complex<R>> {
	static constexpr mxComplexity complexFlag = mxCOMPLEX;
};
...
template<typename R>
struct MatlabNum<std::complex<R>> 
	: IsComplexScalar<std::complex<R>>
{
	static const mxClassID id = MatlabNum<R>::id;
};

template<>	struct MatlabNum<bool> : IsComplexScalar<bool> { static const mxClassID id = mxLOGICAL_CLASS; };
template<>	struct MatlabNum<char> : IsComplexScalar<char> { static const mxClassID id = mxCHAR_CLASS; };
template<>	struct MatlabNum<int> : IsComplexScalar<int> { static const mxClassID id = mxINT32_CLASS; };
template<>	struct MatlabNum<float> : IsComplexScalar<float> { static const mxClassID id = mxSINGLE_CLASS; };
template<>	struct MatlabNum<double> : IsComplexScalar<double> { static const mxClassID id = mxDOUBLE_CLASS; };

template<typename R>
inline mxArray* createMatlabArray(const mwSize *dims, int ndim)
{	return mxCreateNumericArray(ndim, dims, MatlabNum<R>::id, MatlabNum<R>::complexFlag);	}
